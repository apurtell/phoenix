...
/**
 * Standard replication of the SYSTEM.CATALOG and SYSTEM.CHILD_LINK table can be dangerous because
 * schemas may change between the source and target clusters at different times, in particular
 * during cluster upgrades. However, tenant-owned data such as tenant-owned views need to be copied.
 * This WALEntryFilter will only allow tenant-owned rows in SYSTEM.CATALOG to be replicated. Data
 * from all other tables is automatically passed.
 */
public class SystemCatalogWALEntryFilter implements WALEntryFilter, WALCellFilter {
  /**
   * This is an optimization to just skip the cell filter if we do not care about cell filter for
   * certain WALEdits.
   */
  ...
  public WAL.Entry filter(WAL.Entry entry) {
    ...
    skipCellFilter = !(SchemaUtil.isMetaTable(entry.getKey().getTableName().getName())
  ...
  /**
   * Is the cell for system child link a tenant owned. This happens if the tenant id is leading in
  ...
  /**
   * SYSTEM.CHILD_LINK table have tenant owned data for parent child links. In this case, the column
   * qualifier is {@code PhoenixDatabaseMetaData#LINK_TYPE_BYTES} and value is
   * {@code PTable.LinkType.CHILD_TABLE}. For corresponding delete markers the KeyValue type
  ...
  public static boolean isCellChildLinkToTenantView(final Cell cell) {
    ...
    boolean isChildLink = CellUtil.matchingQualifier(cell, PhoenixDatabaseMetaData.LINK_TYPE_BYTES);
    if (
      ...
      /**
       * if the child link is to a tenant-owned view, the COLUMN_NAME field will be the byte[] of
       * the tenant otherwise, it will be an empty byte array (NOT QueryConstants.SEPARATOR_BYTE,
       * but a byte[0]). This assumption is also true for child link's delete markers in
       * SYSTEM.CHILD_LINK as it only contains link rows and does not deal with other type of rows
       * like column rows that also has COLUMN_NAME populated with actual column name.
       **/
