...
/**
 * Command-line tool for analyzing Phoenix Replication Log files. This tool can: - Read a single log
 * file or directory of log files - Print file headers, trailers, and block headers - Decode and
...
public class LogFileAnalyzer extends Configured implements Tool {
  ...
  private void analyzeFile(Path file) throws IOException {
    ...
    LogFileReaderContext context = new LogFileReaderContext(getConf()).setFileSystem(fs)
      .setFilePath(file).setSkipCorruptBlocks(check); // Skip corrupt blocks if checking
    ...
    try {
      ...
      if (trailer != null) {
        ...
        System.out.println("  Blocks Start Offset: " + reader.getTrailer().getBlocksStartOffset());
        System.out
          .println("  Trailer Start Offset: " + reader.getTrailer().getTrailerStartOffset());
      } else {
        System.out.println("\nTrailer is null");
      }
      ...
      if (check) {
        ...
        System.out.println("  Corrupt Blocks Skipped: " + context.getCorruptBlocksSkipped());
