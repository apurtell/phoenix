...
public class GlobalIndexCheckerWithRegionMovesIT extends BaseTest {
  ...
  private static class TestScanningResultPostDummyResultCaller
    ...
    public void postDummyProcess() {
      if (
        hasTestStarted && (countOfDummyResults++ % 4) == 0
  ...
  protected static void moveRegionsOfTable(String tableName) throws IOException {
    ...
    regionsOnServer1.forEach(regionInfo -> {
      if (regionInfo.getTable().equals(TableName.valueOf(tableName))) {
        try {
          for (int i = 0; i < 2; i++) {
            RegionStatesCount regionStatesCount = admin.getClusterMetrics()
              .getTableRegionStatesCount().get(TableName.valueOf(tableName));
            if (
              regionStatesCount.getRegionsInTransition() == 0
                && regionStatesCount.getOpenRegions() == regionStatesCount.getTotalRegions()
            ) {
              LOGGER.info("Moving region {} to {}", regionInfo.getRegionNameAsString(), server2);
              admin.move(regionInfo.getEncodedNameAsBytes(), server2);
              break;
            } else {
              LOGGER.info("Table {} has some region(s) in RIT or not online", tableName);
            }
          }
        } catch (IOException e) {
          throw new RuntimeException(e);
        ...
        try {
          for (int i = 0; i < 2; i++) {
            ...
            if (
              ...
              LOGGER.info("Moving region {} to {}", regionInfo.getRegionNameAsString(), server1);
              break;
            } else {
              LOGGER.info("Table {} has some region(s) in RIT or not online", tableName);
            }
  ...
  public void testDelete() throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      populateTable(dataTableName); // with two rows ('a', 'ab', 'abc', 'abcd') and ('b', 'bc',
                                    // 'bcd', 'bcde')
      ...
      conn.createStatement().execute("CREATE INDEX " + indexTableName + " on " + dataTableName
        + " (val1) include (val2, val3)" + (async ? "ASYNC" : "") + this.indexDDLOptions);
  ...
  public void testPhoenixRowTimestamp() throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement()
        .execute("CREATE INDEX " + indexTableName + " on " + dataTableName
          + " (val1, PHOENIX_ROW_TIMESTAMP()) " + "include (val2, val3) " + (async ? "ASYNC" : "")
          + this.indexDDLOptions);
      if (async) {
        // Run the index MR job to rebuild the index and verify that index is built correctly
        IndexToolIT.runIndexTool(false, null, dataTableName, indexTableName, null, 0,
      ...
      conn.createStatement().execute(
        "CREATE INDEX " + indexTableName + " on " + dataTableName + " (PHOENIX_ROW_TIMESTAMP()) "
          + "include (val1, val2, val3) " + (async ? "ASYNC" : "") + this.indexDDLOptions);
      if (async) {
        // Run the index MR job to rebuild the index and verify that index is built correctly
        IndexToolIT.runIndexTool(false, null, dataTableName, indexTableName, null, 0,
          IndexTool.IndexVerifyType.AFTER);
      ...
      indexTableName = generateUniqueName();
      conn.createStatement().execute("CREATE INDEX " + indexTableName + " on " + dataTableName
        + " (PHOENIX_ROW_TIMESTAMP())" + (async ? "ASYNC" : "") + this.indexDDLOptions);
      if (async) {
        // Run the index MR job to rebuild the index and verify that index is built correctly
        IndexToolIT.runIndexTool(false, null, dataTableName, indexTableName, null, 0,
          IndexTool.IndexVerifyType.AFTER);
  ...
  public void testIndexRowWithoutEmptyColumn() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement()
        .execute("upsert into " + dataTableName + " values ('a', 'abc','abcc', 'abccd')");
      commitWithException(conn);
      // The above upsert will create an unverified index row with row key {'abc', 'a'} and will
      // make the existing
      // index row with row key {'ab', 'a'} unverified
      // Configure IndexRegionObserver to allow the data write phase
      IndexRegionObserver.setFailDataTableUpdatesForTesting(false);
      // Do major compaction which will remove the empty column cell from the index row with row key
      // {'ab', 'a'}
  ...
  public void testLimitWithUnverifiedRows() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      TABLE_NAMES.add(indexTableName);
      conn.commit();
      // Read all index rows and rewrite them back directly. This will overwrite existing rows with
      // newer
      // timestamps and set the empty column to value "x". This will make them unverified
      conn.createStatement()
        .execute("UPSERT INTO " + indexTableName + " SELECT * FROM " + indexTableName);
      conn.commit();
      ...
      conn.createStatement()
        .execute("upsert into " + dataTableName + " (id, val1, val2) values ('c', 'aa','cde')");
      commitWithException(conn);
      IndexRegionObserver.setFailDataTableUpdatesForTesting(false);
      // Verified that read repair will not reduce the number of rows returned for LIMIT queries
  ...
  public void testSimulateConcurrentUpdates() throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      // Configure IndexRegionObserver to fail the last write phase (i.e., the post index update
      // phase) where the
      // verify flag is set to true and/or index rows are deleted and check that this does not
      // impact the
      // correctness.
      IndexRegionObserver.setFailPostIndexUpdatesForTesting(true);
      ...
      conn.commit();
      // The expected state of the index table is {('aa', 'a', 'abcc', 'abcd'), ('bc', 'b', 'bcd',
      // 'bcde')}
      // Do more multiple updates on the same data row
      conn.createStatement()
        .execute("upsert into " + dataTableName + " (id, val1, val3) values ('a', null, null)");
      conn.createStatement()
        .execute("upsert into " + dataTableName + " (id, val1) values ('a', 'ab')");
      conn.createStatement()
        .execute("upsert into " + dataTableName + " (id, val1) values ('b', 'ab')");
      conn.createStatement()
        .execute("upsert into " + dataTableName + " (id, val1, val2) values ('b', 'ab', null)");
      conn.commit();
  ...
  public void testFailPostIndexDeleteUpdate() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      // This DML will scan the Index table and detect unverified index rows. This will trigger read
      // repair which
      // result in deleting these rows since the corresponding data table rows are deleted already.
      // So, the number of
      // rows to be deleted by the "DELETE" DML will be zero since the rows deleted by read repair
      // will not be visible
      // to the DML
  ...
  public void testPartialRowUpdateForMutable() throws Exception {
    ...
    conn.createStatement()
      .execute("upsert into " + dataTableName + " (id, val1, val3) values ('a', 'ab', 'abcdd')");
    conn.commit();
    // Verify that we will read from the index table
    assertExplainPlan(conn, selectSql, dataTableName, indexTableName);
    rs = conn.createStatement().executeQuery(selectSql);
  ...
  /**
   * Phoenix allows immutable tables with indexes to be overwritten partially as long as the indexed
   * columns are not updated during partial overwrites. However, there is no check/enforcement for
   * this. The immutable index mutations are prepared at the client side without reading existing
   * data table rows. This means the index mutations prepared by the client will be partial when the
   * data table row mutations are partial. The new indexing design assumes index rows are always
   * full and all cells within an index row have the same timestamp. On the read path,
   * GlobalIndexChecker returns only the cells with the most recent timestamp of the row. This means
   * that if the client updates the same row multiple times, the client will read back only the most
   * recent update which could be partial. To support the partial updates for immutable indexes,
   * GlobalIndexChecker allows cells with different timestamps to be be returned to the client for
   * immutable tables even though this breaks the design assumption. This test is to verify the
   * partial row update support for immutable tables.
   * <p>
   * Allowing partial overwrites on immutable indexes is a broken model in the first place. Assuring
   * correctness in the presence of failures does not seem possible without using something similar
   * to the solution for mutable indexes.
   * <p>
   * Immutable indexes should be used only for really immutable tables and for these tables,
   * overwrites should be due to failures and should be idempotent. For everything else, mutable
  ...
  public void testPartialRowUpdateForImmutable() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement().execute("CREATE INDEX " + indexTableName + " on " + dataTableName
        + " (val1) include (val2, val3)" + this.indexDDLOptions);
      conn.createStatement()
        .execute("upsert into " + dataTableName + " (id, val1, val2) values ('a', 'ab', 'abcc')");
      conn.commit();
      String selectSql = "SELECT * from " + dataTableName + " WHERE val1  = 'ab'";
      // Verify that we will read from the index table
      assertExplainPlan(conn, selectSql, dataTableName, indexTableName);
  ...
  public void testFailPreIndexRowUpdate() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      // Configure IndexRegionObserver to fail the first write phase (i.e., the pre index update
      // phase). This should not
  ...
  public void testFailPostIndexRowUpdate() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      IndexRegionObserver.setFailPostIndexUpdatesForTesting(false);
      String selectSql =
        "SELECT val2, val3 from " + dataTableName + " WHERE val1  = 'ab' and val2 = 'abcc'";
      // Verify that we will read from the index table
      assertExplainPlan(conn, selectSql, dataTableName, indexTableName);
      ResultSet rs = conn.createStatement().executeQuery(selectSql);
      assertTrue(rs.next());
  ...
  public void testOnePhaseOverwiteFollowingTwoPhaseWrite() throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      populateTable(dataTableName); // with two rows ('a', 'ab', 'abc', 'abcd') and ('b', 'bc',
                                    // 'bcd', 'bcde')
      String indexTableName = generateUniqueName();
      conn.createStatement().execute("CREATE INDEX " + indexTableName + "1 on " + dataTableName
        + " (val1) include (val2, val3)" + (async ? "ASYNC" : "") + this.indexDDLOptions);
      conn.createStatement().execute("CREATE INDEX " + indexTableName + "2 on " + dataTableName
        + " (val2) include (val1, val3)" + (async ? "ASYNC" : "") + this.indexDDLOptions);
      TABLE_NAMES.add(dataTableName);
      ...
      IndexRegionObserver.setFailDataTableUpdatesForTesting(true);
      conn.createStatement()
        .execute("upsert into " + dataTableName + " values ('c', 'cd', 'cdee', 'cdfg')");
      commitWithException(conn);
  ...
  public void testOnePhaseOverwrite() throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      // Configure IndexRegionObserver to skip the last two write phase (i.e., the data table update
      // and post index
      // update phase) and check that this does not impact the correctness (one overwrite)
      IndexRegionObserver.setFailDataTableUpdatesForTesting(true);
      IndexRegionObserver.setFailPostIndexUpdatesForTesting(true);
      conn.createStatement().execute(
        "upsert into " + dataTableName + " (id, val2, val3) values ('a', 'abcc', 'abccc')");
      commitWithException(conn);
      IndexRegionObserver.setFailDataTableUpdatesForTesting(false);
      IndexRegionObserver.setFailPostIndexUpdatesForTesting(false);
      // Read only one column and verify that this is sufficient for the read repair to fix
      // all the columns of the unverified index row that was generated due to doing only one phase
      // write above
  ...
  private void createTableAndIndexes(Connection conn, String dataTableName, String indexTableName)
    throws Exception {
    createTableAndIndexes(conn, dataTableName, indexTableName, 1);
  }

  private void createTableAndIndexes(Connection conn, String dataTableName, String indexTableName,
    int indexVersions) throws Exception {
    populateTable(dataTableName); // with two rows ('a', 'ab', 'abc', 'abcd') and ('b', 'bc', 'bcd',
                                  // 'bcde')
    conn.createStatement()
      .execute("CREATE INDEX " + indexTableName + "1 on " + dataTableName
        + " (val1) include (val2, val3)" + (async ? "ASYNC" : "") + " VERSIONS=" + indexVersions
        + (Strings.isNullOrEmpty(this.indexDDLOptions) ? "" : "," + this.indexDDLOptions));
    conn.createStatement()
      .execute("CREATE INDEX " + indexTableName + "2 on " + dataTableName
        + " (val2) include (val1, val3)" + (async ? "ASYNC" : "") + " VERSIONS=" + indexVersions
        + (Strings.isNullOrEmpty(this.indexDDLOptions) ? "" : "," + this.indexDDLOptions));
    conn.commit();
    if (async) {
      // run the index MR job.
  ...
  public void testFailDataTableAndPostIndexRowUpdate() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      // Configure IndexRegionObserver to fail the last two write phase (i.e., the data table update
  ...
  public void testUnverifiedIndexRowWithFilter() throws Exception {
    ...
    props.setProperty(QueryServices.PHOENIX_SERVER_PAGING_ENABLED_ATTRIB, String.valueOf(false));
    try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
      ...
      conn.commit();
      conn.createStatement().execute(
        "create index " + indexName + " ON " + dataTableName + "(name) include (status, val)");
      conn.createStatement()
        .execute("upsert into " + dataTableName + " values (1, 'tom', 1, 'blah')");
      conn.createStatement()
        .execute("upsert into " + dataTableName + " values (2, 'jerry', 2, 'jee')");
      conn.commit();
      ...
      conn.createStatement()
        .execute("upsert into " + dataTableName + " (id, status) values (1, 2)");
      commitWithException(conn);
      ...
      // new row (3, 'tom', 1)
      conn.createStatement()
        .execute("upsert into " + dataTableName + " (id, name, status) values (3, 'tom', 1)");
      commitWithException(conn);
  ...
  public void testUnverifiedIndexRowWithSkipScanFilter() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement()
        .execute("upsert into " + dataTableName + " (id, val1) values ('b', 'bcc')");
      commitWithException(conn);
      ...
      conn.createStatement()
        .execute("upsert into " + dataTableName + " (id, val3) values ('b', 'bcdf')");
      commitWithException(conn);
  ...
  public void testUnverifiedIndexRowWithFirstKeyOnlyFilter() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      populateTable(dataTableName); // with two rows ('a', 'ab', 'abc', 'abcd') and ('b', 'bc',
                                    // 'bcd', 'bcde')
      conn.createStatement().execute("CREATE INDEX " + indexName + " on " + dataTableName
        + " (val1, id, val2, val3) " + this.indexDDLOptions);
      conn.commit();
      ...
      // Verify that we will read from the index table with a first-key-only/empty-column
      // filter. V1 emits `RANGE SCAN` with the trailing constraint kept in a server filter;
      // V2 emits `SKIP SCAN ON 1 KEY` with the trailing constraint consumed into the scan
      // bounds. Either scan shape is correct; the subsequent query-result assertion (below)
      // is the source of truth for correctness.
      ExplainPlanAttributes attributes = getExplainAttributes(conn, selectSql);
      assertPlan(attributes).scanType(isV2Optimizer() ? "SKIP SCAN ON 1 KEY" : "RANGE SCAN")
        .table(indexName).serverProjectionFilter(encoded);
  ...
  public void testViewIndexRowUpdate() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement()
        .execute("CREATE VIEW " + viewName + " (id char(10) not null, "
          + "val2 varchar, val3 varchar, CONSTRAINT pk PRIMARY KEY (id)) AS SELECT * FROM "
          + dataTableName + " WHERE kp = '0EC'");
      // Create an index on the view
      String indexName = generateUniqueName();
      TABLE_NAMES.add(dataTableName);
      TABLE_NAMES.add("_IDX_" + dataTableName);
      conn.createStatement().execute("CREATE INDEX " + indexName + " on " + viewName
        + " (val2) include (val3)" + this.indexDDLOptions);
      Properties props = new Properties();
      ...
      try (Connection tenantConn = DriverManager.getConnection(getUrl(), props)) {
        ...
        tenantConn.createStatement().execute(
          "CREATE VIEW " + childViewName + " (zid CHAR(15)) " + "AS SELECT * FROM " + viewName);
        // Create another view of the child view
        String grandChildViewName = generateUniqueName();
        tenantConn.createStatement().execute("CREATE VIEW " + grandChildViewName
          + " (val4 CHAR(15)) " + "AS SELECT * FROM " + childViewName);
        IndexRegionObserver.setFailPostIndexUpdatesForTesting(true);
