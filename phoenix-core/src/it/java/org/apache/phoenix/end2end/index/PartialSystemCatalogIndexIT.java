...
public class PartialSystemCatalogIndexIT extends ParallelStatsDisabledIT {
  ...
  private final static String SYS_INDEX_TABLE_LINK_TEST_INDEX_SQL =
    String.format("CREATE INDEX IF NOT EXISTS %s "
  ...
  private final static String SYS_INDEX_HDR_TEST_INDEX_SQL =
    String.format("CREATE INDEX IF NOT EXISTS %s "
  ...
  private final static String SYS_VIEW_HDR_TEST_INDEX_SQL = String.format(
    "CREATE INDEX IF NOT EXISTS %s "
      ...
      + "INCLUDE (TABLE_TYPE, VIEW_STATEMENT, TTL, ROW_KEY_MATCHER) " + "WHERE TABLE_TYPE = 'v'",
    SYS_VIEW_HDR_TEST_INDEX_NAME);
  private final static String SYS_ROW_KEY_MATCHER_TEST_INDEX_SQL = String.format(
    "CREATE INDEX IF NOT EXISTS %s "
      ...
      + "INCLUDE (VIEW_STATEMENT) " + "WHERE TABLE_TYPE = 'v' AND ROW_KEY_MATCHER IS NOT NULL",
  ...
  private final static String SYS_VIEW_INDEX_HDR_TEST_INDEX_SQL = String.format(
    "CREATE INDEX IF NOT EXISTS %s "
      ...
      + "WHERE TABLE_TYPE = 'i' AND LINK_TYPE IS NULL AND VIEW_INDEX_ID IS NOT NULL",
    SYS_VIEW_INDEX_HDR_TEST_INDEX_NAME);

  // SQLs on the data table - SYSTEM.CATALOG
  static final String SYS_CATALOG_ROW_KEY_MATCHER_HEADER_SQL =
    "SELECT ROW_KEY_MATCHER FROM SYSTEM.CATALOG "
  ...
  static final String SYS_CATALOG_VIEW_INDEX_HEADER_SQL =
    "SELECT VIEW_INDEX_ID FROM SYSTEM.CATALOG "
      + "WHERE %s AND TABLE_SCHEM = '%s' AND TABLE_NAME = '%s' AND TABLE_TYPE = 'i' AND LINK_TYPE IS NULL";

  static final String SYS_CATALOG_SYS_INDEX_TABLE_SQL =
    "SELECT count(*) FROM SYSTEM.CATALOG " + "WHERE TABLE_SCHEM = 'SYSTEM' AND TABLE_NAME = '%s'";
  ...
  static final String SYS_CATALOG_IDX_INDEX_HDR_SQL =
    "SELECT \"0:DATA_TABLE_NAME\", \"0:INDEX_STATE\" FROM %s "
      + "WHERE %s AND \":TABLE_SCHEM\" = '%s' AND \":TABLE_NAME\" = '%s'";
  ...
  void assertSystemCatalogHasIndexHdr(String tenantId, String schemaName, String tableName)
    throws SQLException {
  ...
  void assertAdditionalColumnInMetaIndexTable(String schemaName, String indexName,
    String newColumnName, boolean exists) throws SQLException {

    try (Connection connection = DriverManager.getConnection(getUrl())) {
      Statement stmt = connection.createStatement();
      String sql =
        String.format(SYS_CATALOG_COLUMN_EXISTS_SQL, schemaName, indexName, newColumnName);
      stmt.execute(sql);
      ResultSet rs = stmt.getResultSet();
      int numRows = rs.next() ? rs.getInt(1) : 0;
  ...
  void assertSystemCatalogHasIndexTableLinks(String tenantId, String schemaName, String tableName)
    throws SQLException {
    ...
    try (Connection connection = DriverManager.getConnection(getUrl())) {
      ...
      String tenantClause = tenantId == null || tenantId.isEmpty()
        ...
        : String.format("TENANT_ID = '%s'", tenantId);
      String sql =
        String.format(SYS_CATALOG_INDEX_TABLE_LINK_SQL, tenantClause, schemaName, tableName);
      stmt.execute(sql);
      ResultSet rs = stmt.getResultSet();
      int numRows = rs.next() ? rs.getInt(1) : 0;
  ...
  void assertSystemCatalogHasViewIndexHeaderRelatedColumns(String tenantId, String schemaName,
    ...
    try (Connection connection = DriverManager.getConnection(getUrl())) {
      ...
      String tenantClause = tenantId == null || tenantId.isEmpty()
        ...
        : String.format("TENANT_ID = '%s'", tenantId);
      String sql =
        String.format(SYS_CATALOG_VIEW_INDEX_HEADER_SQL, tenantClause, schemaName, tableName);
      stmt.execute(sql);
      ResultSet rs = stmt.getResultSet();
  ...
  void assertSystemCatalogHasViewHeaderRelatedColumns(String tenantId, String schemaName,
    ...
    try (Connection connection = DriverManager.getConnection(getUrl())) {
      ...
      String tenantClause = tenantId == null || tenantId.isEmpty()
        ...
        : String.format("TENANT_ID = '%s'", tenantId);
      String sql =
        String.format(SYS_CATALOG_VIEW_TTL_HEADER_SQL, tenantClause, schemaName, tableName);
      stmt.execute(sql);
      ResultSet rs = stmt.getResultSet();
      if (exists) {
        ...
        assertFalse(
          String.format("Rows do exists for schema = %s, table = %s", schemaName, tableName),
          rs.next());
  ...
  void assertSystemCatalogIndexTable(String systemCatalogIndexName, boolean exists)
    throws SQLException {
  ...
  void assertSystemCatalogIndexHaveIndexHdr(String systemCatalogIndexName, String tenantId,
    String schemaName, String tableName, boolean exists, String indexName,
    String expectedIndexState) throws SQLException {
    ...
    try (Connection connection = DriverManager.getConnection(getUrl())) {
      ...
      if (exists) {
        ...
        assertNull(
          String.format("Zero rows expected for schema = %s, table = %s", schemaName, indexName),
          actualDataTableName);
        assertNull(
          String.format("Zero rows expected for schema = %s, table = %s", schemaName, indexName),
          actualIndexState);
  ...
  void assertSystemCatalogIndexHaveIndexTableLinks(String systemCatalogIndexName, String tenantId,
    String schemaName, String tableName, boolean exists, String indexName) throws SQLException {
    ...
    try (Connection connection = DriverManager.getConnection(getUrl())) {
      ...
      if (exists) {
        ...
        assertNull(
          String.format("Zero rows expected for schema = %s, table = %s", schemaName, tableName),
          colFamilyStr);
      }
    }
  }

  void assertSystemCatalogIndexHaveViewHeaders(String systemCatalogIndexName, String tenantId,
    String schemaName, String tableName, boolean exists) throws SQLException {
  ...
  void assertSystemCatalogIndexHaveViewIndexHeaders(String systemCatalogIndexName, String tenantId,
    String schemaName, String tableName, boolean exists) throws SQLException {
  ...
  void alterIndexState(String indexName, String tableName, PIndexState newState)
    throws SQLException {
    try (Connection conn = DriverManager.getConnection(getUrl());
      Statement stmt = conn.createStatement()) {
  ...
  void dropTableWithChildViews(String baseTable, int numTaskRuns) throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      // Run DropChildViewsTask to complete the tasks for dropping child views. The depth of the
      // view tree is 2,
      // so we expect that this will be done in two task handling runs as each non-root level will
      // be processed
      // in one run
      ...
      assertTaskColumns(conn, PTable.TaskStatus.COMPLETED.toString(),
        PTable.TaskType.DROP_CHILD_VIEWS, null, null, null, null, null);
      ...
      ViewUtil.findAllRelatives(getUtility().getConnection().getTable(linkTable),
        HConstants.EMPTY_BYTE_ARRAY, SchemaUtil.getSchemaNameFromFullName(baseTable).getBytes(),
        SchemaUtil.getTableNameFromFullName(baseTable).getBytes(), PTable.LinkType.CHILD_TABLE,
        childViewsResult);
  ...
  static void assertTaskColumns(Connection conn, String expectedStatus, PTable.TaskType taskType,
    ...
    String expectedIndexName) throws SQLException {
    ResultSet rs = conn.createStatement()
      .executeQuery("SELECT * " + " FROM " + PhoenixDatabaseMetaData.SYSTEM_TASK_NAME + " WHERE "
  ...
  protected PhoenixTestBuilder.SchemaBuilder createLevel1TenantView(
    ...
    PhoenixTestBuilder.SchemaBuilder.TenantViewOptions tenantViewOverrideOptions =
      PhoenixTestBuilder.SchemaBuilder.TenantViewOptions.withDefaults();
    if (tenantViewOptions != null) {
    ...
    schemaBuilder.withTableOptions(tableOptions).withTenantViewOptions(tenantViewOverrideOptions)
  ...
  public void testIndexesOfIndexTableLinkTypeAndIndexHdrCondition() throws Exception {
    ...
    // Create 2 level view
    final PhoenixTestBuilder.SchemaBuilder schemaBuilder =
      createLevel2TenantViewWithGlobalLevelTTL(VIEW_TTL_300_SECS, tenantViewOptions, null, true);
    ...
    assertSystemCatalogHasViewHeaderRelatedColumns("", schemaName, globalViewName, true,
      VIEW_TTL_300_SECS);
    ...
    // Assert index table header rows (table_type = 'i' AND INDEX_STATE IS NOT NULL) exists in
    // SYSTEM. CATALOG
    ...
    assertSystemCatalogIndexHaveIndexTableLinks(FULL_SYS_INDEX_TABLE_LINK_TEST_INDEX_NAME, null,
      schemaName, globalViewName, true, globalIndexName);
    assertSystemCatalogIndexHaveIndexTableLinks(FULL_SYS_INDEX_TABLE_LINK_TEST_INDEX_NAME, tenantId,
      schemaName, tenantViewName, true, tenantIndexName);

    assertSystemCatalogIndexHaveIndexHdr(FULL_SYS_INDEX_HDR_TEST_INDEX_NAME, null, schemaName,
      globalViewName, true, globalIndexName, PIndexState.ACTIVE.getSerializedValue());
    assertSystemCatalogIndexHaveIndexHdr(FULL_SYS_INDEX_HDR_TEST_INDEX_NAME, tenantId, schemaName,
      tenantViewName, true, tenantIndexName, PIndexState.ACTIVE.getSerializedValue());

    // Alter the index state and verify meta index
    alterIndexState(globalIndexName, fullGlobalViewName, PIndexState.UNUSABLE);
    assertSystemCatalogIndexHaveIndexHdr(FULL_SYS_INDEX_HDR_TEST_INDEX_NAME, null, schemaName,
      globalViewName, true, globalIndexName, PIndexState.INACTIVE.getSerializedValue());
    ...
    assertSystemCatalogHasViewHeaderRelatedColumns("", schemaName, globalViewName, false,
      VIEW_TTL_300_SECS);
    assertSystemCatalogHasViewHeaderRelatedColumns(tenantId, schemaName, tenantViewName, false, 0);

    // Assert appropriate rows are dropped/deleted in the SYSTEM.CATALOG index tables
    assertSystemCatalogIndexHaveIndexTableLinks(FULL_SYS_INDEX_TABLE_LINK_TEST_INDEX_NAME, null,
      schemaName, globalViewName, false, null);
    assertSystemCatalogIndexHaveIndexTableLinks(FULL_SYS_INDEX_TABLE_LINK_TEST_INDEX_NAME, tenantId,
      schemaName, tenantViewName, false, null);

    assertSystemCatalogIndexHaveIndexHdr(FULL_SYS_INDEX_HDR_TEST_INDEX_NAME, null, schemaName, null,
      false, globalIndexName, null);
    assertSystemCatalogIndexHaveIndexHdr(FULL_SYS_INDEX_HDR_TEST_INDEX_NAME, tenantId, schemaName,
      null, false, tenantIndexName, null);
  ...
  public void testIndexesOfViewAndIndexHeadersCondition() throws Exception {
    ...
    final PhoenixTestBuilder.SchemaBuilder schemaBuilder =
      createLevel1TenantView(tenantViewOptions, null);
    String tenantId = schemaBuilder.getDataOptions().getTenantId();
    String fullBaseTableName = schemaBuilder.getEntityTableName();
    String schemaName =
      stripQuotes(SchemaUtil.getSchemaNameFromFullName(schemaBuilder.getEntityTenantViewName()));
    ...
    assertSystemCatalogHasViewHeaderRelatedColumns(tenantId, schemaName, tenantViewName, true,
      VIEW_TTL_120_SECS);
    // Assert index header rows (link_type IS NULL AND TABLE_TYPE = 'i') exists in SYSTEM. CATALOG
    assertSystemCatalogHasViewIndexHeaderRelatedColumns(tenantId, schemaName, indexOnTenantViewName,
      true);
    ...
    // Assert appropriate rows are inserted in the SYSTEM.CATALOG index tables
    assertSystemCatalogIndexHaveViewHeaders(FULL_SYS_VIEW_HDR_TEST_INDEX_NAME, tenantId, schemaName,
      tenantViewName, true);
    assertSystemCatalogIndexHaveViewHeaders(FULL_SYS_ROW_KEY_MATCHER_TEST_INDEX_NAME, tenantId,
      schemaName, tenantViewName, true);
    assertSystemCatalogIndexHaveViewIndexHeaders(FULL_SYS_VIEW_INDEX_HDR_TEST_INDEX_NAME, tenantId,
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      String rowKeyMatcherQuery =
        "select ROW_KEY_MATCHER, TTL, TABLE_NAME FROM SYSTEM.CATALOG WHERE TABLE_TYPE = 'v' AND ROW_KEY_MATCHER IS NOT NULL";
      if (isV2Optimizer()) {
        assertPlan(conn, rowKeyMatcherQuery).scanType("SKIP SCAN ON 1 RANGE")
          .tableContains(FULL_SYS_ROW_KEY_MATCHER_TEST_INDEX_NAME).keyRanges("[not null,*,'v']");
      } else {
        assertPlan(conn, rowKeyMatcherQuery).scanType("RANGE SCAN")
          .tableContains(FULL_SYS_ROW_KEY_MATCHER_TEST_INDEX_NAME).keyRanges("[not null]");
      }
    }
    ...
    LOGGER.info("Dropping base table " + fullBaseTableName);
    dropTableWithChildViews(fullBaseTableName, 2);
    // Assert view header rows (link_type IS NULL AND TABLE_TYPE = 'v') does not exist in
    // SYSTEM.CATALOG
    assertSystemCatalogHasViewHeaderRelatedColumns(tenantId, schemaName, tenantViewName, false,
      VIEW_TTL_120_SECS);
    ...
    // Assert index header rows (link_type IS NULL AND TABLE_TYPE = 'i') does not exists in
    // SYSTEM.CATALOG
    assertSystemCatalogHasViewIndexHeaderRelatedColumns(tenantId, schemaName, tenantViewName,
      false);

    // Assert appropriate rows are dropped/deleted in the SYSTEM.CATALOG index tables
    assertSystemCatalogIndexHaveViewHeaders(FULL_SYS_VIEW_HDR_TEST_INDEX_NAME, tenantId, schemaName,
      tenantViewName, false);
    assertSystemCatalogIndexHaveViewHeaders(FULL_SYS_ROW_KEY_MATCHER_TEST_INDEX_NAME, tenantId,
  ...
  public void testIndexesOnOtherSystemTables() throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl());
      ...
      try {
        stmt.execute(
          "CREATE INDEX IF NOT EXISTS SYS_INDEX_LINK_4_IDX ON SYSTEM.CHILD_LINK(TENANT_ID, TABLE_SCHEM, TABLE_NAME, LINK_TYPE) WHERE LINK_TYPE = 4");
        fail();
      ...
      try {
        stmt.execute(
          "CREATE INDEX IF NOT EXISTS SYS_INDEX_STATS_IDX ON SYSTEM.STATS(PHYSICAL_NAME, COLUMN_FAMILY, GUIDE_POST_WIDTH, GUIDE_POSTS_ROW_COUNT) WHERE COLUMN_FAMILY = '4'");
        fail();
      } catch (SQLException sqle) {
        Assert.assertEquals(CANNOT_INDEX_SYSTEM_TABLE.getErrorCode(), sqle.getErrorCode());
      }
      try {
        stmt.execute(
          "CREATE INDEX IF NOT EXISTS SYS_INDEX_LOG_IDX ON SYSTEM.LOG(USER, CLIENT_IP, QUERY) WHERE QUERY_ID = '4'");
        fail();
      } catch (SQLException sqle) {
        Assert.assertEquals(CANNOT_INDEX_SYSTEM_TABLE.getErrorCode(), sqle.getErrorCode());
      }
  ...
  public void testAddColumnWithCascadeOnMetaIndexes() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl());
      Statement stmt = conn.createStatement()) {
      stmt.execute(
        "ALTER TABLE SYSTEM.CATALOG ADD IF NOT EXISTS TEST_COL1 INTEGER DEFAULT 5 CASCADE INDEX ALL");
    }

    // Assert the new column exists in the meta index definition
    assertAdditionalColumnInMetaIndexTable("SYSTEM", SYS_INDEX_TABLE_LINK_TEST_INDEX_NAME,
      "0:TEST_COL1", true);
    assertAdditionalColumnInMetaIndexTable("SYSTEM", SYS_INDEX_HDR_TEST_INDEX_NAME, "0:TEST_COL1",
      true);
