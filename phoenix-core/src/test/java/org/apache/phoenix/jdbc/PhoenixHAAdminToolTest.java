...
public class PhoenixHAAdminToolTest {
  ...
  private final HighAvailibilityCuratorProvider mockHighAvailibilityCuratorProvider =
  ...
  /**
   * Test command line options. In the test body, we split sections by {} to make sure no variable
   * is reused in mistakenly.
   */
  @Test
  ...
  public void testHelpMessage() throws Exception {
    ...
    PhoenixHAAdminTool.OPTIONS.getOptions()
      .forEach((o) -> assertTrue(STDOUT_CAPTURE.toString().contains(((Option) o).getLongOpt())));
  }
  ...
  public void testGetZookeeperQuorum() {
    ...
    final String expectedLocalZk2 =
      String.format("%s:%d:%s", host, port, HConstants.DEFAULT_ZOOKEEPER_ZNODE_PARENT);
