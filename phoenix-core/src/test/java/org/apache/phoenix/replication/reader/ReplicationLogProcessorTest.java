...
public class ReplicationLogProcessorTest extends ParallelStatsDisabledIT {
  ...
  private static final String CREATE_TABLE_SQL_STATEMENT =
    "CREATE TABLE %s (ID VARCHAR PRIMARY KEY, " + "COL_1 VARCHAR, COL_2 VARCHAR, COL_3 BIGINT)";
  ...
  public void testCloseReader() throws IOException {
    ...
    reader.init(new LogFileReaderContext(conf).setFileSystem(localFs).setFilePath(filePath));
  ...
  public void testReplicationLogProcessorConfiguration() throws IOException {
    ...
    assertEquals("Custom batch size bytes should be honored", customBatchSizeBytes,
      customProcessor.getBatchSizeBytes());

    assertEquals("Custom HBase client retries count should be honored", customRetriesCount,
      customProcessor.getHBaseClientRetriesCount());

    assertEquals("Custom HBase client operation timeout should be honored", customOperationTimeout,
      customProcessor.getHBaseClientOperationTimeout());

    assertEquals("Custom batch retry count should be honored", customBatchRetryCount,
      customProcessor.getBatchRetryCount());

    assertEquals("Custom max retry delay should be honored", customMaxRetryDelay,
    ...
    assertEquals("Custom thread pool size should be used", customThreadPoolSize,
  ...
  public void testProcessLogFileWithNonExistentFile() throws Exception {
    ...
    Path nonExistentFilePath =
      new Path(testFolder.getRoot().getAbsolutePath(), "non_existent_log_file.log");
  ...
  public void testProcessLogFileForUnClosedFile() throws Exception {
    ...
    final Path emptyFilePath =
      new Path(testFolder.newFile("testProcessLogFileForUnClosedFile").toURI());
    LogFileWriter writer = initLogFileWriter(emptyFilePath);
  ...
  /**
   * Tests batch size logic when both count and size limits are configured. Verifies that batching
  ...
  public void testProcessLogFileBatchSizeLogic() throws Exception {
    ...
    LogFileTestUtil.assertMutationEquals("First batch big mutation mismatch", mutations.get(3),
    ...
    assertEquals("Second batch should have 5 (equal to batch size) mutations (count limit)", 5,
    ...
    assertEquals("Third batch should have 2 mutations (remaining small mutations)", 2,
  ...
  public void testProcessLogFileWithMultipleTables() throws Exception {
    final Path multiTableBatchFilePath =
      new Path(testFolder.newFile("testProcessLogFileWithMultipleTables").toURI());
    ...
    assertEquals("Should have captured " + expectedBatchCalls + " batch calls", expectedBatchCalls,
  ...
  public void testApplyMutationsWithEmptyMap() throws IOException {
    ...
    ReplicationLogProcessor replicationLogProcessor =
      new ReplicationLogProcessor(conf, testHAGroupName);
    try {
      ReplicationLogProcessor.ApplyMutationBatchResult applyMutationBatchResult =
  ...
  /**
   * Tests applyMutations method with three tables where two succeed and one fails. This test
   * verifies that the method properly handles partial failure scenarios and returns only the failed
  ...
  public void testApplyMutationsWithPartialFailures() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      assertNotNull("Apply mutations result exception must not be null",
        applyMutationBatchResult.getException());
      assertTrue("Invalid exception message", applyMutationBatchResult.getException().getMessage()
      ...
      assertEquals("Table3 should have all its mutations failed", mutations3.size(),
      ...
      ReplicationLogProcessorMetricValues metricValues =
        replicationLogProcessor.getMetrics().getCurrentMetricValues();
      assertEquals("Failed mutations count should match table3 mutations", mutations3.size(),
        metricValues.getFailedMutationsCount());
    } finally {
      replicationLogProcessor.close();
    }
  ...
  /**
   * Tests applyMutations method with two tables that both succeed in one go. This test verifies
   * that the method can handle multiple tables successfully and returns an empty failed mutations
   * map.
   */
  @Test
  ...
  /**
   * Tests applyMutations method with two tables that both fail. This test verifies that the method
   * properly handles complete failure scenarios and returns all mutations in the failed mutations
   * map.
   */
  @Test
  public void testApplyMutationsFailure() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      assertEquals("Table1 should have all its mutations failed", table1Mutations.size(),
        failedMutations.get(TableName.valueOf(table1)).size());
      for (int i = 0; i < table1Mutations.size(); i++) {
        LogFileTestUtil.assertMutationEquals("Table1 mutation mismatch", table1Mutations.get(i),
      ...
      assertEquals("Table2 should have all its mutations failed", table2Mutations.size(),
        failedMutations.get(TableName.valueOf(table2)).size());
      for (int i = 0; i < table2Mutations.size(); i++) {
        LogFileTestUtil.assertMutationEquals("Table2 mutation mismatch", table2Mutations.get(i),
      ...
      assertEquals("Failed mutations count mismatch",
        table1Mutations.size() + table2Mutations.size(), metricValues.getFailedMutationsCount());
    } finally {
      replicationLogProcessor.close();
    }
  }
  ...
  /**
   * Tests processReplicationLogBatch method with multiple tables where all mutations succeed in one
   * go.
   */
  @Test
  public void testProcessReplicationLogBatchWithMultipleTablesSuccess() throws Exception {
    ...
    ReplicationLogProcessor.ApplyMutationBatchResult successApplyMutationsResult =
      new ReplicationLogProcessor.ApplyMutationBatchResult(Collections.emptyMap(), null);
    ...
    LogFileTestUtil.assertMutationEquals("Table1 mutation 1 mismatch", mutations1.get(0),
      capturedMutations1.get(0));
    LogFileTestUtil.assertMutationEquals("Table1 mutation 2 mismatch", mutations1.get(1),
    ...
    LogFileTestUtil.assertMutationEquals("Table2 mutation 1 mismatch", mutations2.get(0),
      capturedMutations2.get(0));
    LogFileTestUtil.assertMutationEquals("Table2 mutation 2 mismatch", mutations2.get(1),
  ...
  /**
   * Tests processReplicationLogBatch method with persistent failure for one table. One table's
   * region is brought down to simulate persistent failure while another table succeeds. This tests
  ...
  public void testProcessReplicationLogBatchWithPersistentFailure() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      assertEquals("Should have made " + expectedCalls + " calls to applyMutations", expectedCalls,
        capturedCalls.size());
      ...
      assertEquals("Mismatch in number of mutations for table1", table1Mutations.size(),
        callMutations1.size());
      for (int i = 0; i < table1Mutations.size(); i++) {
        LogFileTestUtil.assertMutationEquals("Mutation mismatch for table1", table1Mutations.get(i),
      ...
      assertEquals("Mismatch in number of mutations for table2", table2Mutations.size(),
        callMutations2.size());
      for (int mutationIndex = 0; mutationIndex < table1Mutations.size(); mutationIndex++) {
        LogFileTestUtil.assertMutationEquals("Mutation mismatch for table2",
          table2Mutations.get(mutationIndex), callMutations2.get(mutationIndex));
      }
      ...
      for (int callIndex = 1; callIndex < capturedCalls.size(); callIndex++) {
        ...
        ReplicationLogProcessorMetricValues metricValues =
          spyProcessor.getMetrics().getCurrentMetricValues();
        assertEquals("Invalid failed mutations count", table2Mutations.size() * 3,
          metricValues.getFailedMutationsCount());
      }
    } finally {
      spyProcessor.close();
  ...
  /**
   * Tests processReplicationLogBatch method with intermittent failure where a region comes back up
   * during retries. This test verifies that the retry logic works correctly when failures are
  ...
  public void testProcessReplicationLogBatchWithIntermittentFailure() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      assertEquals("Mismatch in number of mutations for table1", mutations1.size(),
        firstCallMutations1.size());
      for (int i = 0; i < mutations1.size(); i++) {
        LogFileTestUtil.assertMutationEquals("Mutation mismatch for table1", mutations1.get(i),
      ...
      assertEquals("Mismatch in number of mutations for table2", mutations2.size(),
        firstCallMutations2.size());
      for (int i = 0; i < mutations2.size(); i++) {
        LogFileTestUtil.assertMutationEquals("Mutation mismatch for table2", mutations2.get(i),
  ...
  private void testProcessLogFileBatching(int totalRecords, int batchSize) throws Exception {
    ...
    for (int batchIndex = 0; batchIndex < expectedBatchCalls; batchIndex++) {
      ...
      assertEquals(
        "Batch " + batchIndex + " should have " + expectedMutationsInBatch + " mutations",
  ...
  private LogFileWriter initLogFileWriter(Path filePath) throws IOException {
    ...
    LogFileWriterContext writerContext =
      new LogFileWriterContext(conf).setFileSystem(localFs).setFilePath(filePath);
  ...
  private List<Mutation> generateHBaseMutations(final PhoenixConnection phoenixConnection,
    final int rows, final String tableName, final long timestamp, final String rowKeyPrefix)
    throws Exception {
    List<Mutation> mutations = new ArrayList<>();
  ...
  /**
   * Validates that the given mutations have been correctly applied to the specified table by
   * creating a temporary table, applying the mutations, and comparing the results.
   */
  private void validate(String tableName, List<Mutation> mutations) throws IOException {
    ...
    try (Connection conn = getConnection()) {
      ...
      for (Map.Entry<String, Result> entry : originalResults.entrySet()) {
        ...
        assertEquals("Number of cells should match for row " + rowKey, originalCells.length,
          tempCells.length);
