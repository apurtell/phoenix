...
public class StatementHintsCompilationTest extends BaseConnectionlessQueryTest {
   ...
   * V2 (typically a wider residual filter under V2 because compound emission's residual-pruning is
   * more conservative) branch on this helper.
   */
  ...
  private static QueryPlan compileStatement(String query, List<Object> binds, Integer limit)
    throws SQLException {
  ...
  public void testSelectForceRangeScanForEH() throws Exception {
    ...
    String serverWhereFilter = isV2Optimizer()
      ? "SERVER FILTER BY (ORGANIZATION_ID = '111111111111111' AND SUBSTR(PARENT_ID, 1, 3) = 'foo'"
        + " AND CREATED_DATE >= DATE '2012-11-01 00:00:00.000' AND CREATED_DATE < DATE"
        + " '2012-11-30 00:00:00.000')"
      : "SERVER FILTER BY (CREATED_DATE >= DATE '2012-11-01 00:00:00.000' AND CREATED_DATE < DATE"
        + " '2012-11-30 00:00:00.000')";
    assertPlan(conn, query).scanType("RANGE SCAN").table("EH")
      ...
      .serverFirstKeyOnlyProjection(true).serverWhereFilter(serverWhereFilter)
