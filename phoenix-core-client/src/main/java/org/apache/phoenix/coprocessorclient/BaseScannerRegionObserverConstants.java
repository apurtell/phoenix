...
public class BaseScannerRegionObserverConstants {
  public enum ReplayWrite {
    ...
    public static ReplayWrite fromBytes(byte[] replayWriteBytes) {
      ...
      if (
        Bytes.compareTo(BaseScannerRegionObserverConstants.REPLAY_TABLE_AND_INDEX_WRITES,
          replayWriteBytes) == 0
      ) {
        return TABLE_AND_INDEX;
      }
      if (
        Bytes.compareTo(BaseScannerRegionObserverConstants.REPLAY_ONLY_INDEX_WRITES,
          replayWriteBytes) == 0
      ) {
        return INDEX_ONLY;
      }
      if (
        Bytes.compareTo(BaseScannerRegionObserverConstants.REPLAY_INDEX_REBUILD_WRITES,
          replayWriteBytes) == 0
      ) {
        return REBUILD_INDEX_ONLY;
      }
      throw new IllegalArgumentException(
        "Unknown ReplayWrite code of " + Bytes.toStringBinary(replayWriteBytes));
    }
  }
   ...
   * presentation. Needed for backward compatibility purposes. TODO: get rid of this in next major
   * release.
   */
  ...
  /**
   * The scan attribute to determine whether client changes are compatible to consume new format
   * changes sent by the server. This attribute is mainly used to address data integrity issues
   * related to region moves (PHOENIX-7106).
   */
