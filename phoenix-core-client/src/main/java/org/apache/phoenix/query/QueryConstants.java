...
public interface QueryConstants {
  ...
  String CREATE_HA_GROUP_METADATA =
    "CREATE TABLE " + SYSTEM_CATALOG_SCHEMA + ".\"" + SYSTEM_HA_GROUP_TABLE + "\"(\n" +
    // PK column
      HA_GROUP_NAME + " VARCHAR NOT NULL," + // This is the unique identifier for the HA group
      // Non-PK columns
      ZK_URL_1 + " VARCHAR," + // This will be used to associate the ClusterRole and URL with local
                               // or peer cluster
      ZK_URL_2 + " VARCHAR," + // This will be used to associate the ClusterRole and URL with local
                               // or peer cluster
      CLUSTER_URL_1 + " VARCHAR," + // This will be returned to the client
      CLUSTER_URL_2 + " VARCHAR," + // This will be returned to the client
      CLUSTER_ROLE_1 + " VARCHAR," + // Role for peer cluster might not be accurate, we will use
                                     // only
                                     // local role for recovery if needed
