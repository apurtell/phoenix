 ...
 * from the cache but cannot construct or close an HA group instance. The reason is that HA group is
 * a shared resource by many clients. Closing it intentionally or accidentally by a client will
...
public class HighAvailabilityGroup {
  ...
  public static final String PHOENIX_HA_ZK_RETRY_MAX_KEY = PHOENIX_HA_ATTR_PREFIX + "zk.retry.max";
  ...
  public static final RetryPolicy RETRY_POLICY =
    new ExponentialBackoffRetry(PHOENIX_HA_ZK_RETRY_BASE_SLEEP_MS_DEFAULT,
      PHOENIX_HA_ZK_RETRY_MAX_DEFAULT, PHOENIX_HA_ZK_RETRY_MAX_SLEEP_MS_DEFAULT);
  ...
  /**
   * Two maps to store client provided info mapping to HighAvailabilityGroup. GROUPS which store
   * HAGroupInfo (name and url of clusters where CRR resides) to HighAvailabilityGroup mapping,
   * which is the information required to get roleRecord and URLS which store HAGroupInfo to
   * HAURLInfo (name, principal) 1:n mapping which represents a given group of clients trying to
   * connect to a HighAvailabilityGroup, this info is required to fetch the CQSI(s) linked to given
   * HighAvailabilityGroup in case of failover or a change where CQSIs needs to be closed and
   * invalidated HAURLInfo is stored in {@link ParallelPhoenixContext} and
   * {@link FailoverPhoenixContext} for the current given connection
   */
  @VisibleForTesting
  ...
  static final Cache<String,
    CuratorFramework> CURATOR_CACHE = CacheBuilder.newBuilder()
      ...
      .removalListener(
        (notification) -> ((CuratorFramework) Objects.requireNonNull(notification.getValue()))
          .close())
      .build();
  /**
  ...
  /**
   * Get an instance of HAURLInfo given the HA connecting URL (with "|") and client properties. Here
   * we do parsing of url and try to extract principal and other additional params
   */
    ...
    if (extraIdx != -1) {
      ...
      if (extraIdx != idx + 1) {
        throw new SQLExceptionInfo.Builder(SQLExceptionCode.MALFORMED_CONNECTION_URL)
          .setMessage(String.format("URL %s is not a valid HA connection string", url)).build()
          .buildException();
      }
   ...
   * <p>
   * The HA group does not have a public constructor. This method is the only public one for getting
   * an HA group instance. The reason is that, HA group is considered expensive to create and
   * maintain. Caching it will make it reusable for all connection requests to this group.
   * <p>
   * It will return the cached instance, if any, for the target HA group. The HA group creation and
   * initialization are blocking operations. Upon initialization failure, the HA group information
   * may be saved in a negative cache iff the cause is due to missing cluster role records. In
   * presence of empty (not null or exception) return value, client may choose to fall back to a
   * single cluster connection to compensate missing cluster role records.
   ...
   * <p>
   * When getting HA group using {@link #get(String, Properties)}, it may return empty (not null or
   * exception) value. In that case client may choose to fall back to a single cluster connection to
   * compensate missing cluster role records instead of throw errors.
   * @param url        The HA connection url optionally; empty optional if properties disables
   *                   fallback
   * @param properties The client connection properties
   * @return The connection url of the single cluster to fall back on, with a fully qualified JDBC
   *         protocol
  ...
  static Optional<String> getFallbackCluster(String url, Properties properties)
    ...
    if (StringUtils.isEmpty(fallbackCluster)) {
      LOG.error(
        "Fallback to single cluster is enabled for the HA group {} but cluster key is"
          ...
          + "ClusterRoleRecords which are source of truth. HA url: '{}'.",
    ...
    if (!fallbackCluster.startsWith(PhoenixRuntime.JDBC_PROTOCOL_ZK)) {
      fallbackCluster =
        PhoenixRuntime.JDBC_PROTOCOL_ZK + PhoenixRuntime.JDBC_PROTOCOL_SEPARATOR + fallbackCluster;
    }

    LOG.info("Falling back to single cluster '{}' for the HA group {} to serve HA connection "
      + "request against url '{}'.", fallbackCluster, haGroupInfo.getName(), url);
      ...
      return CURATOR_CACHE.get(cacheKey, () -> {
        ...
        try {
          if (
            ...
            throw new RuntimeException("Failed to connect to the CuratorFramework in " + "timeout "
  ...
  private static CuratorFramework createCurator(String jdbcUrl, Properties properties,
    ...
    String connectionTimeoutMsProp =
      properties.getProperty(PHOENIX_HA_ZK_CONNECTION_TIMEOUT_MS_KEY);
  ...
  public void init() throws IOException {
    ...
    PairOfSameType<HAClusterRoleManager> newRoleManagers =
      new PairOfSameType<>(new HAClusterRoleManager(info.urls.getFirst(), properties),
   ...
   * <p>
   * After returning, the HA state might not be in READY state. That is possible when a new ZK node
   * change is detected triggering HA group to become IN_TRANSIT state.
    ...
    try {
      ...
      if (state == State.READY && url.isPresent()) {
        ...
        if (state == State.READY && isActive) {
          ...
          conn.close();
          throw new SQLExceptionInfo.Builder(SQLExceptionCode.HA_CLOSED_AFTER_FAILOVER)
            .setMessage("Cluster is not active any more in HA group. Please retry.")
            .setHaGroupInfo(info.toString()).build().buildException();
        }
      } else {
        LOG.error("Not able to connect to active cluster, state: {}, active exist: {}", state,
          url.isPresent());
        throw new SQLExceptionInfo.Builder(SQLExceptionCode.HA_NO_ACTIVE_CLUSTER)
  ...
  boolean isActive(PhoenixConnection connection) {
    ...
    return roleRecord.getActiveUrl().equals(Optional.of(JDBCUtil.formatUrl(connection.getURL())));
  }
   ...
   * connection to the given cluster without checking the context of the cluster's role. Please use
   * {@link #connectActive(Properties, HAURLInfo)} to connect to the ACTIVE cluster.
   */
  PhoenixConnection connectToOneCluster(String url, Properties properties, HAURLInfo haurlInfo)
    ...
    if (!role.canConnect()) {
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.HA_CLUSTER_CAN_NOT_CONNECT)
        .setMessage("Can not connect to cluster '" + url + "' in '" + role + "' role").build()
        .buildException();
    }
   ...
   * methods to public. Constructor is also private (or package-private visible for testing). The
  ...
  void close() {
    ...
    try {
      ...
      if (
        !roleManagerExecutor.awaitTermination(PHOENIX_HA_ZK_SESSION_TIMEOUT_MS_DEFAULT,
          TimeUnit.MILLISECONDS)
      ) {
  ...
  private synchronized boolean applyClusterRoleRecord(@NonNull ClusterRoleRecord newRoleRecord) {
    if (roleRecord == null) {
      ...
      state = State.READY;
      LOG.info("HA group {} is now in {} state after getting initial V{} role record: {}", info,
        state, roleRecord.getVersion(), roleRecord);
    ...
    try {
      ...
      LOG.error("HA group {} failed to transit cluster roles per policy {} to new record {}", info,
    ...
    roleRecord = newRoleRecord;
    state = State.READY;
    LOG.info("HA group {} is in {} state after applying V{} role record. Old: {}, new: {}", info,
      state, roleRecord.getVersion(), oldRecord, roleRecord);
    LOG.debug("HA group is ready: {}", this);
    return true;
  }

  /**
   * Local state of this HA group object, which transits upon explicit call (e.g. init) or when the
   * cluster role change is detected.
   * <p>
   * - UNINITIALIZED is the state when this HA group has not been initialized. Once the HA group is
   * initialized, it will never go to this state again. - READY is the state when this HA group can
   * serve client request. There is not necessarily an active HBase cluster since a standby cluster
   * may be sufficient per HA policy. - IN_TRANSITION is the state where HA group is dealing with
   * cluster role changes and all client connection requests are rejected. - CLOSED is the state
   * where the HA group is closed. Once the HA group is closed, it will never leave this state.
   */
  enum State {
    UNINITIALIZED,
    READY,
   ...
   * Objects of this class are used as the keys of HA group cache {@link #GROUPS} and HA url info
   * cache {@link #URLS}.
   * <p>
  ...
  static final class HAGroupInfo {
    ...
    public boolean equals(Object other) {
      ...
      return new EqualsBuilder().append(name, otherInfo.name).append(urls, otherInfo.urls)
        .isEquals();
    }

    @Override
    public int hashCode() {
      return new HashCodeBuilder(17, 37).append(name).append(urls).hashCode();
    }
  }

  /**
   * Helper method to construct the jdbc url back from the given information for a HAGroup Url are
   * expected to be passed in correct format i.e.
   * zk1\\:port1,zk2\\:port2,zk3\\:port3,zk4\\:port4,zk5\\:port5::znode or
   * master1\\:port1,master2\\:port2,master3\\:port3,master4\\:port4,master5\\:port5
   * @param url       contains host and port part of jdbc url, to get ha url in jdbc format this
   *                  function can be used, but needs url in ha format already i.e. [url1|url2] for
   *                  MASTER and RPC registry
   ...
   *         example :-
   *         jdbc:phoenix+zk:zk1\\:port1,zk2\\:port2,zk3\\:port3,zk4\\:port4,zk5\\:port5::znode:principal:additionalParams
   *         or
   *         jdbc:phoenix+master:master1\\:port1,master2\\:port2,master3\\:port3,master4\\:port4,master5\\:port5::principal:additionParams
   */
  public static String getJDBCUrl(String url, HAURLInfo haURLInfo,
    ...
    if (haURLInfo != null) {
      if (ObjectUtils.anyNotNull(haURLInfo.getPrincipal(), haURLInfo.getAdditionalJDBCParams())) {
        ...
        sb.append(haURLInfo.getPrincipal() == null
   ...
   * <p>
   * It is a runnable to keep setting up the curator and the node cache. It will also register the
   * node watcher so any znode data change will trigger a callback function updating HA group.
   */
  private final class HAClusterRoleManager implements Runnable {
    ...
    public void run() {
      ...
      while (!Thread.currentThread().isInterrupted()) {
        try {
          ...
          LOG.warn("HA cluster role manager thread for '{}' is interrupted, exiting", jdbcUrl, e);
          break;
        } catch (Throwable t) {
    ...
    private void nodeChanged() {
      ...
      if (applyClusterRoleRecord(newRecord)) {
        LOG.info(
          "Successfully apply new cluster role record from cluster '{}', " + "new record: {}",
          jdbcUrl, newRecord);
