...
public class MetricsIndexerSourceImpl extends BaseSourceImpl implements MetricsIndexerSource {
  ...
  public MetricsIndexerSourceImpl(String metricsName, String metricsDescription,
    ...
    indexPrepareTimeHisto =
      getMetricsRegistry().newHistogram(INDEX_PREPARE_TIME, INDEX_PREPARE_TIME_DESC);
    slowIndexPrepareCalls =
      getMetricsRegistry().newCounter(SLOW_INDEX_PREPARE, SLOW_INDEX_PREPARE_DESC, 0L);
    indexWriteTimeHisto =
      getMetricsRegistry().newHistogram(INDEX_WRITE_TIME, INDEX_WRITE_TIME_DESC);
    slowIndexWriteCalls =
      getMetricsRegistry().newCounter(SLOW_INDEX_WRITE, SLOW_INDEX_WRITE_DESC, 0L);
    preWALRestoreTimeHisto =
      getMetricsRegistry().newHistogram(PRE_WAL_RESTORE_TIME, PRE_WAL_RESTORE_TIME_DESC);
    slowPreWALRestoreCalls =
      getMetricsRegistry().newCounter(SLOW_PRE_WAL_RESTORE, SLOW_PRE_WAL_RESTORE_DESC, 0L);
    ...
    postDeleteTimeHisto =
      getMetricsRegistry().newHistogram(POST_DELETE_TIME, POST_DELETE_TIME_DESC);
    slowPostDeleteCalls =
      getMetricsRegistry().newCounter(SLOW_POST_DELETE, SLOW_POST_DELETE_DESC, 0L);
    ...
    slowDuplicateKeyCalls =
      getMetricsRegistry().newCounter(SLOW_DUPLICATE_KEY, SLOW_DUPLICATE_KEY_DESC, 0L);
    replicationSyncTimeHisto =
      getMetricsRegistry().newHistogram(REPLICATION_SYNC_TIME, REPLICATION_SYNC_TIME_DESC);

    postIndexUpdateTimeHisto =
      getMetricsRegistry().newHistogram(POST_INDEX_UPDATE_TIME, POST_INDEX_UPDATE_TIME_DESC);
    preIndexUpdateTimeHisto =
      getMetricsRegistry().newHistogram(PRE_INDEX_UPDATE_TIME, PRE_INDEX_UPDATE_TIME_DESC);
    postIndexUpdateFailureTimeHisto = getMetricsRegistry()
      .newHistogram(POST_INDEX_UPDATE_FAILURE_TIME, POST_INDEX_UPDATE_FAILURE_TIME_DESC);
    preIndexUpdateFailureTimeHisto = getMetricsRegistry()
      .newHistogram(PRE_INDEX_UPDATE_FAILURE_TIME, PRE_INDEX_UPDATE_FAILURE_TIME_DESC);
    postIndexUpdateFailures = getMetricsRegistry().newCounter(POST_INDEX_UPDATE_FAILURE,
      POST_INDEX_UPDATE_FAILURE_DESC, 0L);
    preIndexUpdateFailures =
      getMetricsRegistry().newCounter(PRE_INDEX_UPDATE_FAILURE, PRE_INDEX_UPDATE_FAILURE_DESC, 0L);
  }
